...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onStart() {
        super.onStart();
        final TextView onlineCounter = (TextView) findViewById(R.id.countOfUsers);
        server = new Server(new Consumer<Pair<String, String>>() {
        ...
        }, new Consumer<String>() { //Для домашнего задания
            @Override
            ...
            public void accept(final Integer count) {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                            if (count != null)
                                onlineCounter.setText(String.format("Пользователей онлайн: %d", count));
                    }
                });
            }
        }
    ...
    /**
     * Функция отображения экземпляра Toast с именем подключившегося пользователя
     * @param userName: имя подключившегося пользователя
     */
    public void showToast(String userName) {
        Toast.makeText(chatWindow.getContext(),
                userName + " подключился к чату",
